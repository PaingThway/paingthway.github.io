<img src="http://paingthway.github.io/paingthway.png" alt="RPB Photo" align="right" width="30%"/>

# _PAING THWAY_
> _PAGG_<br />
> Payap University, Department of Computer Science<br />
> Email: paingthway74@gmail.com | Phone: 0954527474<br />

> Motivated IT student at Payap University with a strong interest in computer science, programming, and data analysis. Transfer student with basic knowledge of software development and problem-solving skills. Eager to learn and gain practical experience in the IT field.

# Education
* Payap University, Chiang Mai, Thailand
* Bachelor of Information Technology (Transfer Student)
* Expected Graduation: (2027)

# Skills
* Programming: Python, Java, SQL (basic)
* Tools: Visual Studio Code, MySQL, Power BI (basic)
* Basic knowledge of machine learning concepts
* Data analysis and problem solving
* Microsoft Office (Word, Excel, PowerPoint)
  
# Projects
* _Plant Health Classification Project_
* Built a machine learning model to classify plant health using environmental data
* Used Python and basic SVM algorithm

* _Street Light Energy Saving System_
* Developed a concept for smart street lighting based on environmental conditions
* Analyzed data to improve energy efficiency

# Experience
* Student Projects and Coursework
* Completed multiple academic projects in programming and data analysis
* Worked with datasets and basic machine learning models
  
# Languages
* English (Advanced)
* Burmese (Native)

# Interests
* Machine learning and data science
* Software development
* Technology and innovation

# Contact Info
* [Web Page](https://paingthway.github.io)
* [LinkedIn](https://www.linkedin.com/in/paing-thway-234a54287?utm_source=share_via&utm_content=profile&utm_medium=member_ios)
* [TikTok](https://www.tiktok.com/@carry_pagg?_r=1&_t=ZS-95AKQORKGsm)
* [Facebook](https://www.facebook.com/share/18axJaGu2y/?mibextid=wwXIfr)
